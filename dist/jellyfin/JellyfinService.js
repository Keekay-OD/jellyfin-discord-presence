...
export const JellyfinService = {
    async GetSessions() {
        try {
            const res = await axios.get(`${process.env.JELLYFIN_URL}/Sessions?activeWithinSeconds=960`, {
                headers: {
                    "Authorization": `MediaBrowser Client="Jellyfin Discord RPC", Device="Windows", DeviceId="DiscordRPC123", Version="1.0", Token="${process.env.JELLYFIN_API_KEY}"`
                }
            });
            return res.data;
        }
    ...
    async GetMySession() {
        ...
        // Find YOUR session
        const session = all.find((s) => s.UserId === userId);
        if (!session) {
